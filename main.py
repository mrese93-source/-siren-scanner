...
def strength_from_score(score):
    if score >= 10:
        return "\U0001f525\U0001f525\U0001f525\U0001f525 קריטי!"
    if score >= 7:
        return "\U0001f525\U0001f525\U0001f525 חזק מאוד"
    return "\U0001f525\U0001f525 חזק"
...
def send_fast_alert(symbol, price, change_1m, change_3m, change_5m, signals, score):
    ...
    direction = "\U0001f7e2 LONG" if is_long else "\U0001f534 SHORT"
    ...
    msg = (
        ...
        + f"\U0001f4b5 מחיר: ${price}\n"
        + f"\U0001f4c9 Funding: {round(funding, 4)}% ({funding_state})\n"
        + f"\U0001f4ca OI: ${round(oi / 1_000_000, 3)}M"
    )
    send_telegram(msg)
...
def send_slow_alert(symbol, price, change_15m, change_1h, change_24h):
    with lock:
        ...
        oi = meta.get("oi", 0)
        funding = meta.get("funding", 0)
    direction = "\U0001f7e2 LONG" if (change_1h > 0 or change_24h > 0) else "\U0001f534 SHORT"
    max_move = max(abs(change_15m), abs(change_1h), abs(change_24h))
    if max_move >= 50:
        strength = "\U0001f525\U0001f525\U0001f525\U0001f525 תנועה חריגה מאוד"
    elif max_move >= 30:
        strength = "\U0001f525\U0001f525\U0001f525 חזק מאוד"
    else:
        strength = "\U0001f525\U0001f525 חזק"
    signals = []
    if abs(change_15m) >= MOVE_15M:
        signals.append(f"⏳ 15m: {round(change_15m, 1)}%")
    if abs(change_1h) >= MOVE_1H:
        signals.append(f"\U0001f550 1h: {round(change_1h, 1)}%")
    if abs(change_24h) >= MOVE_24H:
        signals.append(f"\U0001f4c5 24h: {round(change_24h, 1)}%")
    funding_state = "Short pays Long" if funding < 0 else "Long pays Short"
    msg = (
        f"{direction} <b>{symbol}</b>\n"
        f"{strength}\n\n"
...
def send_accumulation_alert(symbol, price, direction, oi_trend, funding_trend, vol_spike, funding):
    funding_state = "Short pays Long" if funding < 0 else "Long pays Short"
    dir_txt = "\U0001f7e2 LONG מוקדם" if direction == "long" else "\U0001f534 SHORT מוקדם"
    signals = [
        f"\U0001f50d OI עולה בעקביות: {round(oi_trend, 2)}% ממוצע",
        "\U0001f9d8 מחיר יציב - צבירה לפני מהלך",
    ]
    if vol_spike >= VOLUME_SPIKE_MIN:
        signals.insert(1, f"\U0001f4e6 Volume קפץ: x{round(vol_spike, 1)} מהרגיל")
    if abs(funding_trend) >= FUNDING_TREND_MIN:
        signals.insert(1, f"\U0001f4c9 Funding מתחיל לזוז: {round(funding_trend, 4)}%")
    msg = (
        f"{dir_txt} <b>{symbol}</b>\n"
        "\U0001f4a1 זיהוי מוקדם - לפני שהמחיר זז\n\n"
        "<b>סיגנלים:</b>\n"
        + "\n".join("- " + s for s in signals)
        + "\n\n"
        + f"\U0001f4b5 מחיר: ${price}\n"
...
def send_funding_extreme_alert(symbol, funding, price):
    direction = "\U0001f7e2 Bias LONG" if funding < 0 else "\U0001f534 Bias SHORT"
    state = "Short pays Long" if funding < 0 else "Long pays Short"
    msg = (
        f"{direction} <b>{symbol}</b>\n"
        "⚠️ <b>Funding חריג</b>\n\n"
        f"\U0001f4c9 Funding: <b>{round(funding, 4)}%</b>\n"
        f"\U0001f9fe מצב: {state}\n"
        f"\U0001f4b5 מחיר: ${price}\n"
    )
    send_telegram(msg)
...
def check_signals(symbol, price):
    ...
    if change_1m >= MOVE_EXPLOSIVE_1M:
        signals.append(f"\U0001f4a3\U0001f4a3\U0001f4a3 +{round(change_1m, 1)}% בדקה (PUMP)")
        score += 8
    elif change_1m >= MOVE_1M:
        signals.append(f"\U0001f680 +{round(change_1m, 1)}% בדקה")
        score += 4
    if change_3m >= MOVE_3M:
        signals.append(f"\U0001f4c8 +{round(change_3m, 1)}% ב-3 דקות")
        score += 3
    if change_5m >= MOVE_5M:
        signals.append(f"⚡ +{round(change_5m, 1)}% ב-5 דקות")
        score += 2
    if change_1m <= -MOVE_EXPLOSIVE_1M:
        signals.append(f"\U0001f4a3\U0001f4a3\U0001f4a3 {round(change_1m, 1)}% בדקה (DUMP)")
        score += 8
    elif change_1m <= -MOVE_1M:
        signals.append(f"\U0001f4a5 {round(change_1m, 1)}% בדקה")
        score += 4
    if change_3m <= -MOVE_3M:
        signals.append(f"\U0001f4c9 {round(change_3m, 1)}% ב-3 דקות")
        score += 3
    if change_5m <= -MOVE_5M:
        signals.append(f"⚡ {round(change_5m, 1)}% ב-5 דקות")
        score += 2
    ...
    if abs(funding) >= FUNDING_ABS_FOR_BOOST and score >= 4:
        signals.append(f"\U0001f4c9 Funding חריג: {round(funding, 4)}%")
        score += 2
