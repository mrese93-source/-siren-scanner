...
SCAN_INTERVAL_SEC = 60            # FASTER: every 1 minute (was 3)
LOOKBACK_5M_SEC = 300            # NEW: 5min for scalping
LOOKBACK_15M_SEC = 900           # 15min for swing
LOOKBACK_1H_SEC = 3600           # 1H for trend confirmation
ALERT_COOLDOWN_SEC = 4 * 60 * 60 # 4 hours (was 6)
...
EXTREME_MOVE_1H = 15.0           # Already moved too much in 1H - too late
PUMP_24H_PCT = 25.0              # Higher threshold (was 20)
DUMP_24H_PCT = -25.0

# Volume - MORE AGGRESSIVE

VOL_DELTA_MIN_USD = 150_000      # Lower threshold (was 200k)
VOL_SPIKE_EXTREME = 8.0          # NEW: Insane spike
VOL_SPIKE_STRONG = 5.0
VOL_SPIKE_MED = 3.0

# Movement - SCALPING FOCUSED

MOVE_EXPLOSIVE = 8.0             # NEW: Explosive move
MOVE_STRONG = 4.0                # Lower (was 5)
MOVE_MED = 2.0                   # Lower (was 3)

# Funding

FUNDING_HIGH = 0.10
FUNDING_EXTREME = 0.15           # NEW: Too crowded
FUNDING_TREND = 0.05

# NEW: Price Acceleration (speed of movement)

ACCELERATION_THRESHOLD = 1.5     # Price moving 1.5x faster than before

# NEW: Large Order Detection

LARGE_ORDER_MIN_USD = 500_000    # Whale detected
...
```
# Compare recent 5min speed vs previous 10min speed
recent_5m = price_change_over_lookback(prices_deque, now_ts, 300)  # 5min
prev_10m = price_change_over_lookback(prices_deque, now_ts - 300, 600)  # 5-15min ago

if abs(prev_10m) < 0.5:  # No movement before
    return 1.0
...
recent_high = max(recent_prices[:-5])  # Excluding very recent
recent_low = min(recent_prices[:-5])

# Bullish break: New high
if curr_price > recent_high * 1.005:  # 0.5% above recent high
    return "bullish_break"

# Bearish break: New low
if curr_price < recent_low * 0.995:  # 0.5% below recent low
...
for t in tickers:
    try:
        ...
        # Large orders (optional - can be slow)
        # trades = get_recent_trades(symbol, 100)
        # buy_vol, sell_vol, net_bias = detect_large_orders(trades, price)
        ...
        # ======================
        # FILTERS - Block obvious bad trades
        # ======================
        
        # Already moved too much in 1H (too late)
        if abs(change_1h) >= EXTREME_MOVE_1H:
            continue
        
        # Funding too extreme (too crowded)
        if abs(funding) >= FUNDING_EXTREME:
            continue
        
        # OI dropping while price moving (unhealthy)
        if oi_trend < -2.0 and abs(change_15m) > 3:
            continue
        
        # Volume dying (fake pump)
        ...
        # Movement (5m AND 15m for confirmation)
        ...
        elif 0 < funding < 0.05:  # Neutral funding = healthy
        ...
        if is_pump or is_dump:
            ...
            if conf < 3:  # Need 3/4 confirmations
                continue
        ...
        alerted[symbol] = now
        found += 1
        print(f"🎯 Alert: {symbol} {direction} score={score} | 5m={round(change_5m,1)}% | accel={round(acceleration,1)}", flush=True)
